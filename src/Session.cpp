...
//Constructor
Session::Session(const std::string &configFilePath):activeUser(), actionsLog(),content(), userMap(){
    //reading from jason
...
//Copy Constructor
Session::Session( const Session &other):activeUser(),content(),actionsLog(),userMap() {
    this->copy(other);
}
//Move Constructor
Session::Session(Session &&other):activeUser(),content(),actionsLog(),userMap() {
    this->move(other);
    // set null
    setOtherNull(other);
}
