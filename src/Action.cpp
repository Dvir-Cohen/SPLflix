...
BaseAction::BaseAction():status(PENDING),username(" "),errorMsg("noError") {}
BaseAction::BaseAction(std::string &username):username(username),status(PENDING),errorMsg("noError"){}
...
void DeleteUser::act(Session &sess) {
    bool found = false;
    for(auto it = sess.getUserMap().begin(); it!=sess.getUserMap().end() && found==false; it++)
    {
        if(it->first.compare(getUserName())==0){
            //detele pointer
            delete(it->second);
            //need to delete it from user map
             sess.getUserMap().erase(it->first);
            found =true;
            complete();
        }
    }
...
void DuplicateUser::act(Session &sess) {
     ...
     bool foundNew = false;
     for(auto it = sess.getUserMap().begin(); it!=sess.getUserMap().end() && !foundNew ; it++)
     {
     ...
     if(foundOrigin && !foundNew){
         //check instance of this user..
         if (origin->getType().compare("len")==0){
            User* newDup=new LengthRecommenderUser(newName);
             // set type!! when building copy assignment
             newDup=origin;
             sess.getUserMap()[newName]=newDup;
         }
         else if(origin->getType().compare("gen")==0){
             User* newDup=new GenreRecommenderUser(newName);
             // set type!! when building copy assignment
             newDup=origin;
             sess.getUserMap()[newName]=newDup;
         }
         else {
             User* newDup=new RerunRecommenderUser(newName);
             // set type!! when building copy assignment
             newDup=origin;
             sess.getUserMap()[newName]=newDup;
         }
...
void Watch::act(Session &sess) {
    ...
    active.addHist(sess.getContent().at(contentId));
    complete();
