...
User::User(const std::string &name):name(name),history(),type("") {}
//copy constructor
User::User(const User &other):name(other.name),type(other.type),history(){
    for(auto &cont:other.history){
        this->history.push_back(cont);
    }
}
//move constructor
User::User(User &&other):name(other.name),type(other.type),history(){
...
std::string User::getType(){ return type;}
void User::setType(std::string str) {type=str;}
